document.addEventListener("DOMContentLoaded", () => {
  ...
  const dom = {
    ...
    onboardingUsernameProxy: document.getElementById(
      "onboarding-username-proxy"
    ),
    onboardingPassword: document.getElementById("onboarding-password"),
    onboardingPasswordProxy: document.getElementById(
      "onboarding-password-proxy"
    ),
    onboardingMasterPassword: document.getElementById(
      "onboarding-master-password"
    ),
    onboardingMasterPasswordProxy: document.getElementById(
      "onboarding-master-password-proxy"
    ),
    ...
    adminMasterPasswordProxy: document.getElementById(
      "admin-master-password-proxy"
    ),
    adminLoginSubmit: document.getElementById("admin-login-submit"),
    switchToUserLoginButtons: document.querySelectorAll(
    ...
    conversationSearchProxy: document.getElementById(
      "conversation-search-proxy"
    ),
    conversationListArea: document.getElementById("conversation-list-area"),
    conversationListPlaceholder: document.getElementById(
    ...
    adminThemeToggleButton: document.getElementById(
  ...
  const showElement = (el) => {
    ...
    const displayStyle =
      ...
      (el.tagName === "SPAN" ||
      el.tagName === "I" ||
      el.classList.contains("custom-button")
        ? "inline-flex"
        : el === dom.confirmationModalOverlay ||
            el.classList.contains("modal-view")
          ? "flex"
          : "flex");
  ...
  const setButtonLoading = (buttonEl, isLoadingFlag) => {
    ...
    if (spinner)
      spinner.style.display = isLoadingFlag ? "inline-block" : "none";
    if (text) text.style.display = isLoadingFlag ? "none" : "inline-block";
    if (icon) {
      if (
        buttonEl.classList.contains("icon-button") ||
        buttonEl.classList.contains("send-button")
      ) {
  ...
  function showError(message, duration = 5000) {
    if (!dom.errorBannerMessage || !dom.errorBanner) return;
    ...
    currentErrorTimeout = setTimeout(hideError, duration);
  }
  function hideError() {
    if (!dom.errorBanner) return;
    dom.errorBanner.classList.remove("show");
    setTimeout(() => {
      if (!dom.errorBanner.classList.contains("show"))
        hideElement(dom.errorBanner);
    }, 300);
  }
  function showConfirmation(message) {
    return new Promise((resolve) => {
      if (
        !dom.confirmationModalOverlay ||
        !dom.confirmationMessage ||
        !dom.confirmNoButton ||
        !dom.confirmYesButton
      ) {
        resolve(false);
        return;
      }
  ...
  function adjustTextareaHeight(textareaProxy = dom.messageInputProxy) {
    ...
    let maxHeight = 120;
    if (textareaProxy.classList.contains("edit-input-contenteditable"))
      maxHeight = 80;
    let baseHeight =
      parseFloat(
        getComputedStyle(textareaProxy).getPropertyValue("min-height")
      ) || 48;
  ...
  function formatLastSeen(isoString, isOnline) {
    ...
    try {
      ...
      const diffSeconds =
        (currentDeviceTime.getTime() - lastActiveDateUTC.getTime()) / 1000;

      if (diffSeconds < 0 && Math.abs(diffSeconds) > 5 * 60) {
        return "Offline";
      }
      ...
      const displayDateDateOnly = new Date(
        displayDate.getFullYear(),
        displayDate.getMonth(),
        displayDate.getDate()
      );
      const currentDeviceDateOnly = new Date(
      ...
      const yesterdayDeviceDateOnly = new Date(
        yesterdayDevice.getFullYear(),
        yesterdayDevice.getMonth(),
        yesterdayDevice.getDate()
      );
  ...
  function updateThemeIcons(theme) {
    ...
    [dom.themeToggleButton, dom.adminThemeToggleButton].forEach((buttonEl) => {
      const icon = buttonEl?.querySelector("i.button-icon");
      if (icon) {
  ...
  function handleThemeToggle() {
    const currentTheme = dom.html.dataset.theme || "dark";
    ...
    try {
      ...
      if (contentType?.includes("application/json")) {
        try {
          ...
          console.error(
            `[API] Invalid JSON from ${endpoint} (status ${response.status}):`,
            e
          );
          if (response.ok)
            return { success: true, warning: "Invalid JSON body." };
      ...
      console.error(
        `[API] ${method} ${endpoint} FAILED (Status: ${status}):`,
        error.message,
        error.data || ""
      );
      if (
        ...
        showError(`Error: ${error.message || "API error occurred."}`);
      }
      throw error;
    }
  }
  ...
  function switchView(newView) {
    ...
    switch (newView) {
      case "loading":
        showElement(dom.loadingScreen);
        break;
      case "onboarding":
        showElement(dom.onboardingScreen);
        dom.onboardingUsernameProxy?.focus();
        break;
      case "login":
        showElement(dom.loginScreen);
        showAdminLoginForm();
        break;
      case "chat":
        showElement(dom.chatAppScreen);
        break;
      case "admin":
        showElement(dom.adminDashboardScreen);
        break;
    }
  ...
  function setFormError(formName, message) {
    const errorElement = dom[`${formName}Error`];
    if (errorElement) {
    ...
    setState({ formErrors: { ...appState.formErrors, [formName]: message } });
  }

  const EMOJI_REGEX_CHECK =
  ...
  function renderConversationList() {
    ...
    if (appState.isLoading.conversations || appState.isLoading.users) {
    ...
    try {
      appState.conversations.forEach((convo) => {
        ...
        if (searchTerm && !partnerUsername.toLowerCase().includes(searchTerm))
          return;
        const lastActive = convo.partner_last_active_ts;
        const isPartnerOnline =
          lastActive &&
          new Date() - new Date(lastActive) <
        ...
        allRenderableItems.push({
          type: "conversation",
          id: convo.id,
          partnerId: convo.partner_id,
          partnerUsername: escapedName,
          lastActiveTs: lastActive,
          lastActivityTs: convo.last_activity_ts || convo.last_message_ts,
          isOnline: isPartnerOnline,
          snippet: snippet,
          timestamp: convo.last_message_ts || convo.last_activity_ts,
      ...
      appState.users.forEach((user) => {
        if (
          ...
          renderedUserIds.has(user.id) ||
          appState.blockedUsers.some((b) => b.id === user.id)
        )
          return;
        ...
        allRenderableItems.push({
          ...
          id: user.id,
          partnerId: user.id,
          partnerUsername: escapeHtml(username),
          lastActiveTs: lastActive,
          lastActivityTs: lastActive,
          isOnline: isOnline,
      ...
      allRenderableItems.sort((a, b) => {
        const aIsActive =
          a.type === "conversation" && a.id === appState.currentConversationId;
        const bIsActive =
          b.type === "conversation" && b.id === appState.currentConversationId;
        if (aIsActive && !bIsActive) return -1;
        if (!aIsActive && bIsActive) return 1;
        if (a.type === "conversation" && b.type === "conversation") {
          const timeA = a.lastActivityTs
            ? new Date(a.lastActivityTs).getTime()
            : 0;
          const timeB = b.lastActivityTs
            ? new Date(b.lastActivityTs).getTime()
            : 0;
          return timeB - timeA;
        }
      ...
      if (allRenderableItems.length === 0) {
        dom.conversationListArea.innerHTML = `<div class="list-placeholder">${searchTerm ? "No matching chats or users found." : "No conversations or users available."}</div>`;
        return;
      }
      ...
      allRenderableItems.forEach((itemData) => {
        ...
        if (itemData.type === "conversation") {
          ...
          if (itemData.id === appState.currentConversationId)
          ...
          const hasConversations = appState.conversations.some(
            (c) =>
              !searchTerm ||
              (c.partner_username || "").toLowerCase().includes(searchTerm)
          );
          if (
            !addedUserHeader &&
            allRenderableItems.filter((i) => i.type === "user").length > 0
          ) {
            const userHeader = document.createElement("p");
            userHeader.textContent = "Start New Chat";
            userHeader.className = "list-separator-header";
            dom.conversationListArea.appendChild(userHeader);
            addedUserHeader = true;
          }
          ...
          item.addEventListener("click", () =>
            handleStartNewConversation(itemData.id)
          );
        }
      ...
      console.error("[Render] renderConversationList - Error:", error);
      dom.conversationListArea.innerHTML = `<div class="list-placeholder error">Error displaying list.</div>`;
    }
  }
  function renderMessages(conversationId) {
    ...
    try {
      const manualButton =
        dom.manualLoadOlderButton &&
        dom.messageArea.contains(dom.manualLoadOlderButton)
          ? dom.manualLoadOlderButton
          : null;
      const olderLoader =
        dom.olderMessagesLoader &&
        dom.messageArea.contains(dom.olderMessagesLoader)
          ? dom.olderMessagesLoader
          : null;
      ...
      if (appState.isLoading.messages && currentMessages.length === 0) {
        ...
        loadingPlaceholder.className = "message-placeholder-style";
        loadingPlaceholder.id = placeholderId;
        loadingPlaceholder.innerHTML = `<i class="fa-solid fa-spinner fa-spin"></i> Loading messages...`;
        dom.messageArea.appendChild(loadingPlaceholder);
        return;
      }
      if (
        ...
        noMessagesPlaceholder.className = "message-placeholder-style";
        noMessagesPlaceholder.id = placeholderId;
        noMessagesPlaceholder.textContent =
          "No messages yet. Send an aura wave!";
        dom.messageArea.appendChild(noMessagesPlaceholder);
        return;
      }
      ...
      const existingStartMarker = dom.messageArea.querySelector(
        `#${startMarkerId}`
      );
      if (
        appState.hasReachedOldestMessage.get(conversationId) &&
        currentMessages.length > 0 &&
        !existingStartMarker
      ) {
        const startMarker = document.createElement("div");
        startMarker.className = "conversation-start-marker";
      ...
      console.error("[Render] renderMessages - Error:", error);
      dom.messageArea.innerHTML = `<div class="message-placeholder-style error">Error displaying messages.</div>`;
    }
  }
  function addMessageToUI(msg, prepend = false, anchorNodeForPrepend = null) {
    if (!dom.messageArea || !appState.currentUser || !msg || !msg.id) {
      return;
    }
    ...
    if (lastTimestamp && !prepend) {
      ...
      if (
        ...
        (msg.sender_id === lastSenderId &&
          timeDiff > TIME_GAP_THRESHOLD_MINUTES * 60 * 1000)
      ) {
    ...
    const isConsecutive =
      ...
      lastTimestamp &&
      currentTimestamp.getTime() - new Date(lastTimestamp).getTime() <
        CONSECUTIVE_MESSAGE_THRESHOLD_MS;
    ...
    const existingElement = dom.messageArea.querySelector(
    ...
    if (existingElement) {
      if (
        msg.is_edited &&
        existingElement.dataset.timestamp !== msg.edited_at
      ) {
        const contentSpan = existingElement.querySelector(".message-content");
        if (contentSpan)
          contentSpan.innerHTML = `${escapeHtml(msg.content)} <span class="edited-indicator">(edited)</span>`;
        existingElement.dataset.timestamp = msg.edited_at;
      }
      if (isMyMessage && !msg.isOptimistic) {
        const currentReadStatus =
          existingElement.querySelector(
            ".message-read-status.read-indicator"
          ) !== null;
        if (
          msg.isReadByPartner !== undefined &&
          msg.isReadByPartner !== currentReadStatus
        ) {
          updateMessageReadStatusUI(existingElement, msg.isReadByPartner);
        }
      }
      if (
        msg.isOptimistic === false &&
        existingElement.classList.contains("sending")
      ) {
        existingElement.classList.remove("sending");
        const actionsContainer =
          existingElement.querySelector(".message-actions");
        if (actionsContainer) {
          actionsContainer.innerHTML = `<button class="message-action-button reply-button" title="Reply"><i class="fa-solid fa-reply"></i></button>${isMyMessage ? '<button class="message-action-button edit-button" title="Edit"><i class="fa-solid fa-pencil"></i></button>' : ""}${isMyMessage ? '<button class="message-action-button delete-button" title="Delete Message"><i class="fa-solid fa-trash-can"></i></button>' : ""}`;
          attachMessageActionListeners(existingElement, msg);
        }
        if (
          isMyMessage &&
          !existingElement.querySelector(".message-read-status")
        ) {
          const metaDiv = existingElement.querySelector(".message-meta");
          const readStatusSpan = document.createElement("span");
          readStatusSpan.className = "message-read-status";
          readStatusSpan.setAttribute("aria-label", "Message status");
          metaDiv?.appendChild(readStatusSpan);
          updateMessageReadStatusUI(
            existingElement,
            msg.isReadByPartner || false
          );
        }
      }
      return;
    ...
    if (isConsecutive && !showTimeGapMarker)
      messageDiv.classList.add("message-consecutive");
    ...
    if (msg.reply_to_message_id) {
      const originalMsg = appState.messages
        .get(msg.conversation_id)
        ?.find((m) => m.id === msg.reply_to_message_id);
      const originalSender = escapeHtml(
        msg.reply_sender_username || originalMsg?.sender_username || "User"
      );
      const snippetText = escapeHtml(
        msg.reply_snippet ||
          originalMsg?.content?.substring(0, 100) ||
          "Original message"
      );
      replyHTML = `<div class="reply-snippet" data-reply-to-id="${msg.reply_to_message_id}" title="Original: ${escapeHtml(msg.reply_snippet || originalMsg?.content || "")}"><strong class="reply-snippet-sender">${originalSender}</strong><span class="reply-snippet-text">${snippetText}</span></div>`;
    }

    const editedIndicatorHTML = msg.is_edited
    ...
    const readStatusHTML =
      isMyMessage && !msg.isOptimistic
        ? '<span class="message-read-status" aria-label="Message status"></span>'
        : "";
    ...
    if (isEmojiOnly) {
      ...
      if (replySnippetDiv)
        replySnippetDiv.style.setProperty("display", "block", "important");
      if (actionsDiv)
        actionsDiv.style.setProperty("display", "flex", "important");
    }

    if (!msg.isOptimistic) {
      attachMessageActionListeners(messageDiv, msg);
    }
    const replySnippetEl = messageDiv.querySelector(".reply-snippet");
    if (replySnippetEl) {
      replySnippetEl.addEventListener("click", (e) => {
        e.stopPropagation();
        const targetMessageId = replySnippetEl.dataset.replyToId;
        scrollToMessage(targetMessageId);
      });
    }
    if (isMyMessage && !msg.isOptimistic) {
      updateMessageReadStatusUI(messageDiv, msg.isReadByPartner || false);
    }
    ...
    if (prepend) {
      if (anchorNodeForPrepend) {
        ...
        while (
          actualFirstChild &&
          (actualFirstChild === dom.manualLoadOlderButton ||
            actualFirstChild === dom.olderMessagesLoader)
        ) {
    ...
    if (replyBtn) {
      ...
      messageDiv
        ...
        .addEventListener("click", (e) => {
          e.stopPropagation();
          handleReplyClick(msg.id, msg.sender_username || "User", msg.content);
        });
    }
    if (editBtn) {
      ...
      messageDiv
        ...
        .addEventListener("click", (e) => {
          e.stopPropagation();
          handleShowEditInput(messageDiv, msg.id, msg.content);
        });
    }
    if (deleteBtn) {
      ...
      messageDiv
        ...
        .addEventListener("click", (e) => {
          e.stopPropagation();
          handleDeleteMessage(msg.conversation_id, msg.id, messageDiv);
        });
    }
  }
  function updateMessageReadStatusUI(messageElement, isReadByPartner) {
    ...
    if (isReadByPartner) {
      statusElement.innerHTML =
        '<i class="fa-solid fa-check-double" title="Read"></i>';
      statusElement.classList.add("read-indicator");
      statusElement.setAttribute("aria-label", "Message read");
    } else {
      statusElement.innerHTML =
        '<i class="fa-solid fa-check" title="Sent"></i>';
      statusElement.classList.add("sent-indicator");
      statusElement.setAttribute("aria-label", "Message sent");
    }
  }
  function removeOptimisticMessage(tempId) {
    const messageElement = dom.messageArea?.querySelector(
      `.message.sending[data-message-id="${tempId}"]`
    );
    if (messageElement) {
      messageElement.remove();
      if (
        dom.messageArea?.querySelectorAll(".message").length === 0 &&
        !appState.isLoading.messages &&
        !appState.isLoading.olderMessages
      ) {
        renderMessages(appState.currentConversationId);
      }
    }
  }
  function updateOptimisticMessage(tempId, confirmedMessage) {
    ...
    if (messageElement) {
      ...
      messageElement.dataset.timestamp =
        confirmedMessage.edited_at || confirmedMessage.timestamp;
      ...
      if (actionsContainer) {
        ...
        attachMessageActionListeners(messageElement, confirmedMessage);
      }
      if (
        confirmedMessage.sender_id === appState.currentUser?.id &&
        !messageElement.querySelector(".message-read-status")
      ) {
        ...
        readStatusSpan.setAttribute("aria-label", "Message status");
        metaDiv?.appendChild(readStatusSpan);
        updateMessageReadStatusUI(messageElement, false);
      }
    }
  }
  function removeMessageUI(conversationId, messageId) {
    if (conversationId !== appState.currentConversationId || !dom.messageArea)
      return;
    const messageElement = dom.messageArea.querySelector(
  ...
  function updateChatHeader(conversation) {
    if (
      ...
      !dom.chatViewHeader ||
      !dom.chatPartnerName ||
      !dom.chatPartnerAvatar ||
      !dom.chatPartnerStatus ||
      !dom.chatPartnerStatusText ||
      !dom.blockUserButton ||
    ...
    const isOnline =
      lastActive &&
      new Date() - new Date(lastActive) < ONLINE_THRESHOLD_MINUTES * 60 * 1000;
    ...
    if (partnerId && !appState.blockedUsers.some((b) => b.id === partnerId)) {
      showElement(dom.blockUserButton);
      dom.blockUserButton.dataset.userId = String(partnerId);
      dom.blockUserButton.title = `Block ${escapeHtml(name)}`;
    } else {
  ...
  function enableChatInput(conversation) {
    ...
    if (!conversation) {
      shouldEnable = false;
      placeholderText = "Select a conversation";
    } else if (
      conversation?.partner_id &&
      appState.blockedUsers.some((u) => u.id === conversation.partner_id)
    ) {
      shouldEnable = false;
      placeholderText = "You have blocked this user.";
    }
  ...
  function clearChatView() {
    ...
    if (
      ...
      dom.messageArea.prepend(dom.manualLoadOlderButton);
    if (
      dom.olderMessagesLoader &&
      !dom.messageArea.contains(dom.olderMessagesLoader)
    ) {
      if (dom.manualLoadOlderButton)
        dom.manualLoadOlderButton.insertAdjacentElement(
          "afterend",
          dom.olderMessagesLoader
        );
    ...
    cancelReply();
    cancelEdit();
    if (dom.messageInputProxy) {
      dom.messageInputProxy.textContent = "";
      dom.messageInputProxy.contentEditable = "false";
      dom.messageInputProxy.dataset.placeholder = "Select a conversation";
      dom.messageInputProxy.classList.remove("has-content");
    }
    if (dom.messageInput) {
    ...
    if (dom.chatPartnerAvatar) {
      dom.chatPartnerAvatar.querySelector("i").className = "fa-solid fa-user";
      dom.chatPartnerAvatar.classList.remove("online", "offline", "group");
      dom.chatPartnerAvatar.dataset.userId = "";
    }
    if (dom.chatPartnerStatus) {
      dom.chatPartnerStatus.dataset.lastActiveTs = "";
      hideElement(dom.chatPartnerStatus);
    }
    if (dom.chatViewHeader) {
      dom.chatViewHeader.dataset.conversationId = "";
      dom.chatViewHeader.dataset.userId = "";
    }
    hideElement(dom.blockUserButton);
  ...
  function renderAdminStats(stats = {}) {
    if (
      !dom.statTotalUsers ||
      !dom.statTotalMessages ||
      !dom.statTotalConversations ||
      !dom.statActiveUsers
    )
      return;
    try {
      ...
      console.error("[Render] renderAdminStats - Error updating UI:", error);
    }
  }
  function renderAdminUserList(adminUsers = []) {
    ...
    try {
      ...
      adminUsers.forEach((user) => {
        ...
        if (deleteBtn) {
          deleteBtn.addEventListener("click", handleDeleteUserByAdmin);
        }
        dom.adminUserListBody.appendChild(row);
      });
    } catch (error) {
      console.error(
        "[Render] renderAdminUserList - Error during rendering:",
        error
      );
      dom.adminUserListBody.innerHTML = `<tr><td colspan="4" class="list-placeholder error">Error displaying users.</td></tr>`;
    }
  }
  function renderBlockedUsersList() {
    ...
    if (appState.isLoading.blockedUsers) {
      dom.blockedUsersList.innerHTML = `<p class="list-placeholder" id="blocked-list-placeholder"><i class="fa-solid fa-spinner fa-spin"></i> Loading...</p>`;
      return;
    }
    if (appState.blockedUsers.length === 0) {
      dom.blockedUsersList.innerHTML = `<p class="list-placeholder" id="blocked-list-placeholder">You haven't blocked anyone.</p>`;
      return;
    }
    appState.blockedUsers.forEach((user) => {
      ...
      if (unblockBtn) {
        unblockBtn.addEventListener("click", handleUnblockUser);
      }
  ...
  function updateActiveStatusUI() {
    ...
    if (dom.myAvatarSummary) {
      dom.myAvatarSummary.classList.remove("offline");
      dom.myAvatarSummary.classList.add("online");
    }
    document
      .querySelectorAll(".conversation-item .avatar[data-user-id]")
      .forEach((avatar) => {
        ...
        if (appState.currentView === "chat") {
          userDataSource =
            appState.conversations.find((c) => c.partner_id === userId) ||
            appState.users.find((u) => u.id === userId);
        } else if (appState.currentView === "admin") {
          userDataSource = appState.adminUserList.find((u) => u.id === userId);
        }
        const lastActiveTs =
          userDataSource?.last_active_ts ||
          userDataSource?.partner_last_active_ts;
        if (lastActiveTs) {
          const isOnline =
            new Date() - new Date(lastActiveTs) < onlineThreshold;
    ...
    if (
      ...
      const currentConvo = appState.conversations.find(
        (c) => c.id === appState.currentConversationId
      );
      const partnerId = currentConvo?.partner_id;
      const lastActiveTs = currentConvo?.partner_last_active_ts;
      if (partnerId) {
        const isOnline =
          lastActiveTs && new Date() - new Date(lastActiveTs) < onlineThreshold;
    ...
    if (appState.currentView === "admin" && dom.adminUserListBody) {
      document
        .querySelectorAll("#admin-user-list-body tr[data-user-id]")
        .forEach((row) => {
          ...
          if (user) {
            ...
            if (lastActiveCell) {
              const isOnline =
                user.last_active_ts &&
                new Date() - new Date(user.last_active_ts) < onlineThreshold;
              lastActiveCell.textContent = formatLastSeen(
                user.last_active_ts,
                isOnline
              );
            }
  ...
  async function checkSetupStatus() {
    try {
      const response = await apiCall("/setup/status");
      return response?.adminExists;
    } catch (error) {
      console.error("[Fetch] checkSetupStatus - Failed:", error.message);
      throw error;
    }
  }
  async function fetchChatData() {
    ...
    let userData = [];
    try {
      blockData = await apiCall("/blocks");
      if (!Array.isArray(blockData)) {
        blockData = [];
      }
    } catch (error) {
      console.error("[Fetch] Fetch blocks failed:", error.message);
    } finally {
      setState({
        blockedUsers: blockData,
        isLoading: { ...appState.isLoading, blockedUsers: false },
      });
      renderBlockedUsersList();
    }
    try {
      convoData = await apiCall("/conversations");
      if (!Array.isArray(convoData)) {
        convoData = [];
      }
    } catch (error) {
      console.error("[Fetch] Fetch conversations failed:", error.message);
    } finally {
      setState({
        conversations: convoData,
        isLoading: { ...appState.isLoading, conversations: false },
      });
    }
    try {
      ...
      console.error("[Fetch] Fetch users failed:", error.message);
    } finally {
      setState({
        users: userData,
        isLoading: { ...appState.isLoading, users: false },
      });
    }
    if (dom.conversationSearchProxy)
      dom.conversationSearchProxy.textContent = "";
    if (dom.conversationSearch) dom.conversationSearch.value = "";
    renderConversationList();
  ...
  async function fetchMessagesForConversation(
    ...
    if (isManualRefresh && dom.refreshMessagesButton) {
      setButtonLoading(dom.refreshMessagesButton, true);
    }
    renderMessages(conversationId);
    try {
      ...
      if (sortedMessages.length > 0) {
        appState.oldestMessageTimestamp.set(
          conversationId,
          sortedMessages[0].timestamp
        );
        if (sortedMessages.length < MESSAGES_INITIAL_LOAD_LIMIT) {
      ...
      markConversationAsRead(
        conversationId,
        fetchedMessages
          .filter((m) => m.sender_id !== appState.currentUser?.id)
          .map((m) => m.id)
      );
      const currentConvoData = appState.conversations.find(
        (c) => c.id === conversationId
      );
      if (currentConvoData) updateChatHeader(currentConvoData);
    } catch (error) {
      console.error(
        `[Fetch] fetchMessagesForConversation - Failed for ${conversationId}:`,
        error.message
      );
      appState.messages.set(conversationId, []);
      appState.hasReachedOldestMessage.set(conversationId, true);
      ...
      if (isManualRefresh && dom.refreshMessagesButton) {
        setButtonLoading(dom.refreshMessagesButton, false);
      }
      renderMessages(conversationId);
      requestAnimationFrame(() => {
      ...
      if (dom.messageArea) {
        dom.messageArea.removeEventListener("scroll", debouncedHandleScroll);
        dom.messageArea.addEventListener("scroll", debouncedHandleScroll);
      }
      updateSendButtonState();
      updateManualLoadButtonVisibility();
    }
  ...
  function updateManualLoadButtonVisibility() {
    ...
    const notCurrentlyLoadingAnyMessages =
      !appState.isLoading.olderMessages && !appState.isLoading.messages;
  ...
  async function fetchOlderMessages(conversationId, isManualClick = false) {
    if (
      ...
      if (isManualClick && dom.manualLoadOlderButton)
        setButtonLoading(dom.manualLoadOlderButton, false);
      updateManualLoadButtonVisibility();
      return;
    }
    ...
    if (isManualClick && dom.manualLoadOlderButton) {
      ...
      if (
        dom.messageArea.firstChild !== dom.olderMessagesLoader &&
        dom.messageArea.firstChild !== dom.manualLoadOlderButton
      ) {
        dom.messageArea.prepend(dom.olderMessagesLoader);
      } else if (
        dom.manualLoadOlderButton &&
        dom.messageArea.firstChild === dom.manualLoadOlderButton &&
        dom.manualLoadOlderButton.nextSibling !== dom.olderMessagesLoader
      ) {
        dom.manualLoadOlderButton.insertAdjacentElement(
          "afterend",
          dom.olderMessagesLoader
    ...
    try {
      const olderFetched = await apiCall(
        `/conversations/${conversationId}/messages?before_ts=${encodeURIComponent(beforeTs)}`
      );
      ...
      if (newOlderMessages.length > 0) {
        ...
        let currentFirstContent = dom.messageArea.querySelector(
          ".message, .conversation-start-marker, .message-time-gap-marker"
        );
        if (
          dom.olderMessagesLoader.nextSibling &&
          (dom.olderMessagesLoader.nextSibling.classList.contains("message") ||
            dom.olderMessagesLoader.nextSibling.classList.contains(
              "conversation-start-marker"
            ) ||
            dom.olderMessagesLoader.nextSibling.classList.contains(
              "message-time-gap-marker"
            ))
        ) {
          currentFirstContent = dom.olderMessagesLoader.nextSibling;
        } else if (
          dom.manualLoadOlderButton.nextSibling &&
          (dom.manualLoadOlderButton.nextSibling.classList.contains(
            "message"
          ) ||
            dom.manualLoadOlderButton.nextSibling.classList.contains(
              "conversation-start-marker"
        ...
        requestAnimationFrame(() => {
          const newScrollHeight = dom.messageArea.scrollHeight;
          dom.messageArea.scrollTop =
      ...
      console.error(
        `[Fetch] fetchOlderMessages - Failed for ${conversationId}:`,
        error.message
      );
      appState.hasReachedOldestMessage.set(conversationId, true);
    } finally {
  ...
  async function fetchBlockedUsers() {
    if (appState.isLoading.blockedUsers && appState.currentView !== "chat")
      return;
    setState({ isLoading: { ...appState.isLoading, blockedUsers: true } });
    renderBlockedUsersList();
    let blockedData = [];
    try {
      blockedData = await apiCall("/blocks");
      if (!Array.isArray(blockedData)) {
        console.warn("[Fetch] Blocks not array:", blockedData);
        blockedData = [];
      }
    } catch (error) {
      console.error("[Fetch] Fetch blocks failed:", error.message);
      if (dom.manageBlocksError)
        setFormError("manageBlocks", `Failed to load: ${error.message}`);
      blockedData = [];
    } finally {
      setState({
  ...
  async function fetchAdminStats() {
    if (appState.isLoading.adminStats && appState.currentView !== "admin")
      return;
    setState({ isLoading: { ...appState.isLoading, adminStats: true } });
    let statsData = {};
    try {
      statsData = await apiCall("/admin/stats");
      if (
        typeof statsData !== "object" ||
        statsData === null ||
        Array.isArray(statsData)
      ) {
        console.warn("fetchAdminStats: Received non-object:", statsData);
        statsData = {};
      }
    } catch (error) {
      console.error("fetchAdminStats: Fetch failed:", error.message);
      statsData = {};
    } finally {
      setState({
        adminStats: statsData,
        isLoading: { ...appState.isLoading, adminStats: false },
      });
      renderAdminStats(appState.adminStats);
    }
  }
  async function fetchAdminUsers() {
    if (appState.isLoading.allUsersAdmin && appState.currentView !== "admin")
      return;
    setState({ isLoading: { ...appState.isLoading, allUsersAdmin: true } });
    renderAdminUserList();
    let usersData = [];
    try {
      usersData = await apiCall("/admin/users");
      if (!Array.isArray(usersData)) {
        console.warn("fetchAdminUsers: Received non-array:", usersData);
        usersData = [];
      }
    } catch (error) {
      console.error("fetchAdminUsers: Fetch failed:", error.message);
      if (dom.adminUserError)
        setFormError("adminUser", `Failed to load users: ${error.message}`);
      usersData = [];
    } finally {
      setState({
        adminUserList: usersData,
        isLoading: { ...appState.isLoading, allUsersAdmin: false },
      });
      renderAdminUserList(appState.adminUserList);
      updateActiveStatusUI();
    }
  }
  ...
  async function fetchNewMessages(conversationId) {
    ...
    try {
      const polledMessages = await apiCall(
        `/conversations/${conversationId}/messages?since=${encodeURIComponent(lastTimestamp)}`
      );
      if (polledMessages?.length > 0) {
        ...
        const isNearBottom =
          dom.messageArea &&
          dom.messageArea.scrollTop + dom.messageArea.clientHeight >=
            dom.messageArea.scrollHeight - 150;
        let partnerSentNew = false;
        let newMsgIdsForRead = [];
        polledMessages.forEach((msg) => {
          ...
          if (messageIndex === -1) {
            ...
            if (msg.sender_id !== appState.currentUser.id) {
              partnerSentNew = true;
              newMsgIdsForRead.push(msg.id);
            }
          } else {
            const cachedMsg = existingMessages[messageIndex];
            if (JSON.stringify(cachedMsg) !== JSON.stringify(msg)) {
              existingMessages[messageIndex] = msg;
              addMessageToUI(msg, false);
              updatedExisting = true;
            }
          }
        });
        if (addedNew || updatedExisting) {
          appState.messages.set(conversationId, [...existingMessages]);
        }
        if (addedNew && isNearBottom) {
          scrollToBottom(dom.messageArea);
        }
        if (addedNew) {
          const latestMsg = existingMessages[existingMessages.length - 1];
          updateConversationListSnippet(conversationId, latestMsg);
          if (partnerSentNew) {
            const currentConvoData = appState.conversations.find(
              (c) => c.id === conversationId
            );
            if (currentConvoData) {
              currentConvoData.last_activity_ts = latestMsg.timestamp;
              currentConvoData.partner_last_active_ts = latestMsg.timestamp;
              updateChatHeader(currentConvoData);
            }
          }
        }
        if (newMsgIdsForRead.length > 0)
          markConversationAsRead(conversationId, newMsgIdsForRead);
      }
    } catch (error) {
      console.error(
        `Message polling failed for ${conversationId}:`,
        error.message
      );
    }
  }
  async function markConversationAsRead(conversationId, messageIds = []) {
    if (!conversationId || !appState.currentUser || messageIds.length === 0)
      return;
    try {
      if (appState.ws && appState.ws.readyState === WebSocket.OPEN) {
        ...
        await apiCall(`/conversations/${conversationId}/read`, "POST", {
          message_ids: messageIds,
        });
      }
      const convoIndex = appState.conversations.findIndex(
        (c) => c.id === conversationId
      );
      if (
        convoIndex > -1 &&
        appState.conversations[convoIndex].unread_count > 0
      ) {
        appState.conversations[convoIndex].unread_count = Math.max(
          0,
          appState.conversations[convoIndex].unread_count - messageIds.length
        );
        renderConversationList();
      }
    } catch (error) {
      console.error(
        `Background mark-as-read failed for ${conversationId}:`,
        error.message
      );
    }
  }
  ...
  async function handleOnboardingSubmit(event) {
    ...
    if (!username || username.length < 3) {
      setFormError("onboarding", "Username min 3 chars.");
      dom.onboardingUsernameProxy?.focus();
      return;
    }
    if (!password || password.length < 8) {
      setFormError("onboarding", "Password min 8 chars.");
      dom.onboardingPasswordProxy?.focus();
      return;
    }
    if (!masterPassword || masterPassword.length < 10) {
      setFormError("onboarding", "Master Password min 10 chars.");
      dom.onboardingMasterPasswordProxy?.focus();
      return;
    }
    ...
    try {
      ...
      setFormError("onboarding", error.message || "Setup failed.");
    } finally {
      setState({ isLoading: { ...appState.isLoading, auth: false } });
  ...
  async function handleUserLoginSubmit(event) {
    ...
    if (!username || !password) {
      setFormError("login", "Username and password required.");
      dom.loginUsernameProxy?.focus();
      return;
    }
    setState({ isLoading: { ...appState.isLoading, auth: true } });
    ...
    try {
      ...
      setFormError("login", error.message || "Login failed.");
  ...
  async function handleAdminLoginSubmit(event) {
    ...
    if (!masterPassword) {
      setFormError("login", "Master password required.");
      dom.adminMasterPasswordProxy?.focus();
      return;
    }
    setState({ isLoading: { ...appState.isLoading, auth: true } });
    ...
    try {
      ...
      setFormError("login", error.message || "Admin login failed.");
      setState({ currentUser: null });
      sessionStorage.removeItem(SESSION_STORAGE_KEY);
    } finally {
      setState({ isLoading: { ...appState.isLoading, auth: false } });
  ...
  async function selectConversation(conversationId) {
    ...
    setState({
      ...
      isLoading: { ...appState.isLoading, olderMessages: false },
    });
    ...
    if (conversation) {
      ...
      if (dom.messageArea && isTouchDevice()) {
        dom.messageArea.addEventListener("touchstart", handleTouchStart, {
          passive: true,
        });
        dom.messageArea.addEventListener("touchmove", handleTouchMove, {
          passive: false,
        });
        dom.messageArea.addEventListener("touchend", handleTouchEnd, {
          passive: true,
        });
      }
      ...
      console.error(`Convo data for ID ${id} not found locally.`);
      showError("Could not load conversation data.");
      clearChatView();
    }
  }
  async function handleSendMessage() {
    ...
    if (!content && appState.editingMessageId) {
    ...
    const optimisticMessage = {
      ...
      is_edited: false,
      edited_at: null,
      reply_to_message_id: wasReplyingTo,
      reply_snippet: replyInfo?.snippet,
      reply_sender_username: replyInfo?.sender,
    ...
    try {
      const result = await apiCall(
        ...
        "POST",
        { content: content, reply_to_message_id: wasReplyingTo }
      );
      if (result?.success && result.message) {
        const confirmedMessage = result.message;
        let existingMessages =
          appState.messages.get(appState.currentConversationId) || [];
        existingMessages = existingMessages.filter(
          (m) => m.id !== tempId && m.id !== confirmedMessage.id
        );
        existingMessages.push(confirmedMessage);
        appState.messages.set(appState.currentConversationId, [
          ...existingMessages,
        ]);
      ...
      if (wasReplyingTo) {
        const originalMsg = appState.messages
          ...
          ?.find((m) => m.id === wasReplyingTo);
        if (originalMsg) {
          handleReplyClick(
            wasReplyingTo,
            originalMsg.sender_username,
            originalMsg.content
          );
        }
      }
      ...
      requestAnimationFrame(() => {
        if (
          dom.messageInputProxy &&
          (dom.messageInputProxy.contentEditable === "true" ||
            dom.messageInputProxy.contentEditable === true)
        ) {
          if (
            appState.lastFocusedElement === dom.messageInputProxy ||
            isTouchDevice()
          ) {
            dom.messageInputProxy.focus();
            const sel = window.getSelection();
            const range = document.createRange();
            range.selectNodeContents(dom.messageInputProxy);
  ...
  async function handleDeleteMessage(
    ...
    if (deleteButtonEl) {
      deleteButtonEl.classList.add("disabled");
      deleteButtonEl.setAttribute("aria-disabled", "true");
    }
    try {
      ...
      appState.messages.set(
        conversationId,
        convoMessages.filter((m) => m.id !== messageId)
      );
      const convoIndex = appState.conversations.findIndex(
        (c) => c.id === conversationId
      );
      if (convoIndex > -1) {
        const remainingMessages = appState.messages.get(conversationId) || [];
        const newLastMessage =
          remainingMessages.length > 0
            ? remainingMessages[remainingMessages.length - 1]
            : null;
        updateConversationListSnippet(conversationId, newLastMessage);
      }
    } catch (error) {
      ...
      if (deleteButtonEl) {
        deleteButtonEl.classList.remove("disabled");
        deleteButtonEl.removeAttribute("aria-disabled");
      }
    } finally {
      setState({
        isLoading: { ...appState.isLoading, deletingMessage: false },
      });
    }
  }
  function logout() {
  ...
  function closeManageBlocksModal(event) {
    if (!event) {
      hideElement(dom.manageBlocksModal);
      return;
    }
    if (
      event.target !== dom.manageBlocksModal &&
      !event.target.closest(".modal-close-button") &&
      !(
        event.currentTarget === dom.manageBlocksModal &&
  ...
  async function handleUnblockUser(event) {
    ...
    try {
      ...
      setState({
        blockedUsers: appState.blockedUsers.filter(
          (u) => u.id !== userIdToUnblock
        ),
      });
      renderBlockedUsersList();
      fetchChatData();
      if (appState.currentConversationId) {
        ...
        if (currentConvo?.partner_id === userIdToUnblock) {
          updateChatHeader(currentConvo);
          enableChatInput(currentConvo);
        }
      }
      ...
      setFormError(
        "manageBlocks",
        `Failed to unblock ${username}: ${error.message}`
      );
    } finally {
      const stillExistingButton = dom.manageBlocksModal?.querySelector(
        `.unblock-button[data-user-id="${userIdToUnblock}"]`
      );
      if (stillExistingButton) setButtonLoading(stillExistingButton, false);
    }
  }
  async function handleBlockUser(userIdToBlock, usernameToBlock = "this user") {
    if (
      !appState.currentUser ||
      isNaN(userIdToBlock) ||
      userIdToBlock === appState.currentUser.id
    )
      return;
    if (appState.blockedUsers.some((u) => u.id === userIdToBlock)) {
      showError(`${usernameToBlock} is already blocked.`);
      return;
    }
    const confirmation = await showConfirmation(`Block ${usernameToBlock}?`);
    if (!confirmation) return;
    const headerBlockButton =
      dom.chatViewHeader?.querySelector("#block-user-button");
    const isBlockingFromHeader =
      headerBlockButton &&
      parseInt(headerBlockButton.dataset.userId, 10) === userIdToBlock;
    ...
    try {
      ...
      setState({
        blockedUsers: [
          ...appState.blockedUsers,
          { id: userIdToBlock, username: usernameToBlock },
        ],
      });
      renderBlockedUsersList();
      if (appState.currentConversationId) {
        ...
        if (currentConvo?.partner_id === userIdToBlock) {
          updateChatHeader(currentConvo);
          enableChatInput(currentConvo);
        }
      }
      ...
      showError(`Failed to block ${usernameToBlock}: ${error.message}`);
    } finally {
      setState({ isLoading: { ...appState.isLoading, blockingUser: false } });
      if (isBlockingFromHeader && headerBlockButton)
        setButtonLoading(headerBlockButton, false);
    }
  }
  ...
  async function handleStartNewConversation(partnerId) {
    if (
      !partnerId ||
      !appState.currentUser ||
      partnerId === appState.currentUser.id ||
      appState.isLoading.conversations
    )
      return;
    const existingItem = dom.conversationListArea?.querySelector(
      `.conversation-item[data-partner-id="${partnerId}"]`
    );
    if (existingItem && existingItem.dataset.conversationId) {
      selectConversation(existingItem.dataset.conversationId);
      return;
    }
    const userItem = dom.conversationListArea?.querySelector(
      `.user-list-item[data-user-id="${partnerId}"]`
    );
    ...
    try {
      ...
      showError(`Could not start chat: ${error.message}`);
  ...
  async function handleAdminAddUserSubmit(event) {
    ...
    if (!username || username.length < 3) {
      setFormError("adminUser", "Username min 3 chars.");
      dom.adminAddUsernameProxy?.focus();
      return;
    }
    if (!password || password.length < 8) {
      setFormError("adminUser", "Password min 8 chars.");
      dom.adminAddPasswordProxy?.focus();
      return;
    }
    ...
    try {
      ...
      setFormError("adminUser", error.message || "Failed.");
    } finally {
      setState({ isLoading: { ...appState.isLoading, adminAction: false } });
  ...
  async function handleDeleteUserByAdmin(event) {
    ...
    const confirmed = await showConfirmation(
      `DELETE USER: ${username}? This is permanent.`
    );
    if (!confirmed) return;
    setState({ isLoading: { ...appState.isLoading, adminAction: true } });
    setButtonLoading(button, true);
  ...
  function handleMobileMenuToggle() {
    ...
    if (dom.panelOverlay) {
      dom.panelOverlay.style.display = dom.body.classList.contains(
        "left-panel-active"
      )
        ? "block"
        : "none";
  ...
  function closeMobilePanel() {
    ...
    if (dom.panelOverlay) dom.panelOverlay.style.display = "none";
  }
  function getReplyInfoForOptimistic(replyToId) {
    if (!replyToId || !appState.currentConversationId) return null;
    const messagesInConv =
      appState.messages.get(appState.currentConversationId) || [];
    const originalMsg = messagesInConv.find((m) => m.id === replyToId);
    if (!originalMsg) return null;
    return {
      snippet: originalMsg.content.substring(0, 100),
      sender: originalMsg.sender_username || "User",
    };
  }
  function handleReplyClick(messageId, senderUsername, messageContent) {
    ...
    dom.replyContextText.textContent = escapeHtml(
  ...
  function cancelReply() {
    ...
    if (dom.replyContextArea) hideElement(dom.replyContextArea);
    if (
      dom.messageInputProxy &&
      !appState.editingMessageId &&
      (dom.messageInputProxy.contentEditable === "true" ||
        dom.messageInputProxy.contentEditable === true)
    )
      dom.messageInputProxy.focus();
  ...
  function handleShowEditInput(messageElement, messageId, currentContent) {
    if (
      !messageElement ||
      !appState.currentUser ||
      parseInt(messageElement.dataset.senderId, 10) !== appState.currentUser.id
    )
      return;
    ...
    if (!currentContent.trim()) editInput.classList.remove("has-content");
    else editInput.classList.add("has-content");
    ...
    saveButton.innerHTML =
      '<i class="fa-solid fa-check button-icon"></i><span class="button-text"> Save</span>';
    const cancelButton = document.createElement("div");
    cancelButton.className = "custom-button button-cancel";
    cancelButton.role = "button";
    cancelButton.tabIndex = 0;
    cancelButton.innerHTML =
      '<i class="fa-solid fa-xmark button-icon"></i><span class="button-text"> Cancel</span>';
    ...
    saveButton.addEventListener("click", (e) =>
      handleSaveEdit(e, editInput, saveButton)
    );
    cancelButton.addEventListener("click", cancelEdit);
    editInput.addEventListener("input", () => {
      adjustTextareaHeight(editInput);
      editInput.classList.toggle("has-content", !!editInput.textContent.trim());
    });
  ...
  async function handleSaveEdit(event, editInput, saveButton) {
    ...
    if (!editInput || !saveButton) {
      const container = messageElement.querySelector(".edit-input-container");
      if (!container) {
        cancelEdit();
        return;
      }
      editInput = container.querySelector(".edit-input-contenteditable");
      saveButton = container.querySelector(".custom-button.button-confirm");
      if (!editInput || !saveButton) {
        cancelEdit();
        return;
      }
    ...
    const originalContent = appState.messages
      .get(appState.currentConversationId)
      ?.find((m) => m.id === appState.editingMessageId)?.content;
    if (!newContent) {
    ...
    try {
      const response = await apiCall(
        `/messages/${appState.editingMessageId}`,
        "PATCH",
        { content: newContent }
      );
      if (response.success) {
        const convoMessages =
          appState.messages.get(appState.currentConversationId) || [];
        const msgIndex = convoMessages.findIndex(
          (m) => m.id === appState.editingMessageId
        );
        let editedMessage = null;
        if (msgIndex > -1) {
          convoMessages[msgIndex].content = newContent;
          convoMessages[msgIndex].is_edited = true;
          convoMessages[msgIndex].edited_at =
            response.edited_at || new Date().toISOString();
          editedMessage = convoMessages[msgIndex];
        }
        cancelEdit();
        const contentSpan = messageElement.querySelector(".message-content");
        if (contentSpan)
        ...
        messageElement.dataset.timestamp =
          response.edited_at || new Date().toISOString();
        if (editedMessage) {
          const latestMessageInCache =
            convoMessages.length > 0
              ? convoMessages[convoMessages.length - 1]
              : null;
          if (
            latestMessageInCache &&
            latestMessageInCache.id === appState.editingMessageId
          ) {
            updateConversationListSnippet(
              appState.currentConversationId,
              editedMessage
      ...
      const stillEditing = dom.messageArea.querySelector(
        `.message.editing[data-message-id="${appState.editingMessageId}"]`
      );
      if (stillEditing) {
        const stillSaveButton = stillEditing.querySelector(
          ".custom-button.button-confirm"
        );
        const stillEditInput = stillEditing.querySelector(
          ".edit-input-contenteditable"
        );
        if (stillSaveButton) setButtonLoading(stillSaveButton, false);
        if (stillEditInput) stillEditInput.contentEditable = "true";
      }
    }
  }
  function updateConversationListSnippet(conversationId, latestMessageOrNull) {
    ...
    if (convoIndex > -1) {
      ...
      if (latestMessageOrNull) {
        let snippetPrefix =
          latestMessageOrNull.sender_id === appState.currentUser.id
            ? "You: "
            : "";
        convo.last_message_content = latestMessageOrNull.content;
        convo.last_message_sender =
          latestMessageOrNull.sender_username ||
          (latestMessageOrNull.sender_id === appState.currentUser.id
            ? appState.currentUser.username
            : "User");
        convo.last_message_ts = latestMessageOrNull.timestamp;
        convo.last_activity_ts = latestMessageOrNull.timestamp;
      } else {
        convo.last_message_content = "No messages yet";
        convo.last_message_sender = null;
        convo.last_message_ts = null;
      }
      renderConversationList();
    }
  }
  function scrollToMessage(messageId) {
    ...
    if (messageElement) {
      ...
      showError("Original message not currently loaded.", 3000);
    }
  }
  function handleMessageAreaScroll() {
    ...
    updateManualLoadButtonVisibility();
    if (
      dom.messageArea.scrollTop < SCROLL_LOAD_THRESHOLD &&
      !appState.isLoading.olderMessages &&
      !appState.isLoading.messages &&
      !appState.hasReachedOldestMessage.get(appState.currentConversationId) &&
      dom.manualLoadOlderButton &&
      dom.manualLoadOlderButton.style.display === "none"
    ) {
  ...
  function handleEmojiClick(emoji) {
    if (!dom.messageInputProxy) return;
    insertTextAtCursor(emoji);
    hideEmojiPanel();
    dom.messageInputProxy.focus();
  }
  function insertTextAtCursor(text) {
    if (!dom.messageInputProxy || !dom.messageInputProxy.isContentEditable)
      return;
    dom.messageInputProxy.focus();
    const sel = window.getSelection();
    if (!sel || sel.rangeCount === 0) {
      dom.messageInputProxy.textContent += text;
      dom.messageInputProxy.dispatchEvent(
  ...
  function handleTouchStart(e) {
    ...
    if (!targetEl || appState.editingMessageId) {
      resetSwipeState();
      return;
    }
    appState.swipeState.startX = touch.clientX;
    appState.swipeState.startY = touch.clientY;
    appState.swipeState.currentX = touch.clientX;
    appState.swipeState.currentY = touch.clientY;
    appState.swipeState.messageEl = targetEl;
    appState.swipeState.isSwiping = false;
    appState.swipeState.targetId = targetEl.dataset.messageId;
    appState.swipeState.confirmedSwipe = false;
  }
  function handleTouchMove(e) {
    ...
    if (!appState.swipeState.confirmedSwipe) {
      if (
        Math.abs(deltaX) > 15 &&
        Math.abs(deltaY) < SWIPE_VERTICAL_MAX_DEVIATION
      ) {
        appState.swipeState.confirmedSwipe = true;
    ...
    if (appState.swipeState.confirmedSwipe) {
      ...
      let translateX = 0;
      const isReceived =
        appState.swipeState.messageEl.classList.contains("received");
      const isSent = appState.swipeState.messageEl.classList.contains("sent");
      if (isReceived && deltaX > 0) {
        translateX = Math.min(deltaX, SWIPE_THRESHOLD + 30);
      } else if (isSent && deltaX < 0) {
        translateX = Math.max(deltaX, -(SWIPE_THRESHOLD + 30));
      } else {
        return;
      }
  ...
  function handleTouchEnd(e) {
    if (!appState.swipeState.confirmedSwipe || !appState.swipeState.messageEl) {
      resetSwipeState();
      return;
    }
    const deltaX = appState.swipeState.currentX - appState.swipeState.startX;
    const targetIsReceived =
      appState.swipeState.messageEl.classList.contains("received");
    const targetIsSent =
      appState.swipeState.messageEl.classList.contains("sent");
    if (
      (targetIsReceived && deltaX > SWIPE_THRESHOLD) ||
      (targetIsSent && deltaX < -SWIPE_THRESHOLD)
    ) {
      const msgId = parseInt(appState.swipeState.targetId, 10);
      const msgData = appState.messages
        .get(appState.currentConversationId)
        ?.find((m) => m.id === msgId);
      if (msgData) {
        handleReplyClick(
          msgId,
          msgData.sender_username || "User",
          msgData.content
        );
      }
    }
  ...
  function resetSwipeState() {
    if (appState.swipeState.messageEl) {
      appState.swipeState.messageEl.style.transform = "";
      appState.swipeState.messageEl.classList.remove("swiping");
    }
    appState.swipeState = {
      startX: 0,
      startY: 0,
  ...
  async function initializeApp() {
    ...
    try {
      adminExists = await checkSetupStatus();
    } catch (e) {
    ...
    if (typeof adminExists !== "boolean") {
      showError("Could not determine application status.", 10000);
      hideElement(dom.loadingScreen);
      return;
    }
    ...
    if (savedSession) {
      try {
        ...
        setState({ currentUser: null });
        disconnectWebSocket();
        if (e.status !== 401) switchView("login");
        else if (appState.currentView !== "login") switchView("login");
        return;
      }
    }
    switchView("login");
  ...
  async function initializeChatView() {
    ...
    switchView("chat");
    if (dom.myUsernameSummary)
      dom.myUsernameSummary.textContent = escapeHtml(
        appState.currentUser.username
      );
    if (dom.myAvatarSummary) {
      dom.myAvatarSummary.classList.remove("offline");
  ...
  function initializeCustomInputs() {
    document
      .querySelectorAll(
        '.custom-input[contenteditable="true"], .custom-textarea[contenteditable="true"], .edit-input-contenteditable[contenteditable="true"]'
      )
      .forEach((customEl) => {
        const targetInputId = customEl.dataset.targetInput;
        const actualInput = targetInputId
        ...
        if (actualInput) {
          customEl.addEventListener("input", () => {
            ...
            customEl.classList.toggle(
              "has-content",
              !!customEl.textContent.trim()
            );
            actualInput.dispatchEvent(new Event("input", { bubbles: true }));
            if (
              customEl.classList.contains("custom-textarea") ||
              customEl.classList.contains("edit-input-contenteditable")
            ) {
          ...
          if (customEl.textContent) customEl.classList.add("has-content");
          else customEl.classList.remove("has-content");
        }
        ...
        if (
          ...
          customEl.addEventListener("keydown", (e) => {
            if (e.key === "Enter" && !e.shiftKey) {
              e.preventDefault();
              const submitButton =
                form.querySelector(".custom-button.button-primary") ||
                form.querySelector('[id$="-submit"]');
              if (
                submitButton &&
        ...
        if (wrapper && wrapper.classList.contains("custom-input-wrapper")) {
          customEl.addEventListener("focus", () => {
        ...
        if (customEl.dataset.isPassword !== undefined) {
          customEl.style.webkitTextSecurity = "disc";
        }
      });
  }
  ...
  function connectWebSocket() {
    ...
    appState.ws.onmessage = (event) => {
      try {
        ...
        switch (data.type) {
          case "user_online":
            handleUserOnlineStatusUpdate(data.userId, true, data.timestamp);
            break;
          case "user_offline":
            handleUserOnlineStatusUpdate(data.userId, false, data.timestamp);
            break;
          case "message_read":
    ...
    if (appState.ws) {
      if (appState.ws.readyState === WebSocket.OPEN && appState.currentUser) {
        try {
          ...
          console.warn("Error sending offline status on disconnect:", e);
        }
      }
      appState.ws.close();
      appState.ws = null;
  ...
  function startWebSocketTimers() {
    ...
    wsHeartbeatInterval = setInterval(() => {
      if (
        ...
        appState.ws.send(
          JSON.stringify({ type: "ping", userId: appState.currentUser.id })
        );
        appState.ws.send(
          JSON.stringify({
            type: "status_update",
  ...
  function handleUserOnlineStatusUpdate(userIdToUpdate, isOnline, timestamp) {
    ...
    const convo = appState.conversations.find(
      (c) => c.partner_id === userIdToUpdate
    );
    if (convo) {
      if (
        (isOnline &&
          (!convo.partner_last_active_ts ||
            new Date(userTimestamp) >
              new Date(convo.partner_last_active_ts))) ||
        !isOnline
      ) {
        convo.partner_last_active_ts = userTimestamp;
        userChanged = true;
      }
      if (appState.currentConversationId === convo.id && userChanged) {
    ...
    if (user) {
      if (
        (isOnline &&
          (!user.last_active_ts ||
            new Date(userTimestamp) > new Date(user.last_active_ts))) ||
        !isOnline
      ) {
    ...
    const adminUser = appState.adminUserList.find(
      (u) => u.id === userIdToUpdate
    );
    if (adminUser) {
      if (
        (isOnline &&
          (!adminUser.last_active_ts ||
            new Date(userTimestamp) > new Date(adminUser.last_active_ts))) ||
        !isOnline
      ) {
  ...
  function handleMessageReadStatusUpdate(conversationId, messageIds, readerId) {
    ...
    messageIds.forEach((messageId) => {
      ...
      if (msgIndex > -1) {
        if (
          messagesInConv[msgIndex].sender_id === appState.currentUser?.id &&
          !messagesInConv[msgIndex].isReadByPartner
        ) {
          messagesInConv[msgIndex].isReadByPartner = true;
          const messageEl = dom.messageArea?.querySelector(
            `.message[data-message-id="${messageId}"]`
          );
  ...
  function attachEventListeners() {
    ...
    dom.onboardingSubmit?.addEventListener("click", () =>
      dom.onboardingForm?.requestSubmit
        ? dom.onboardingForm.requestSubmit()
        : dom.onboardingForm?.submit()
    );
    dom.userLoginForm?.addEventListener("submit", handleUserLoginSubmit);
    dom.loginSubmit?.addEventListener("click", () =>
      dom.userLoginForm?.requestSubmit
        ? dom.userLoginForm.requestSubmit()
        : dom.userLoginForm?.submit()
    );
    dom.adminLoginForm?.addEventListener("submit", handleAdminLoginSubmit);
    dom.adminLoginSubmit?.addEventListener("click", () =>
      dom.adminLoginForm?.requestSubmit
        ? dom.adminLoginForm.requestSubmit()
        : dom.adminLoginForm?.submit()
    );
    ...
    document.addEventListener("keydown", (e) => {
      if (e.key === "Escape") {
        if (dom.confirmationModalOverlay?.style.display !== "none") {
          handleConfirmation(false);
        } else if (appState.editingMessageId) {
          cancelEdit();
        } else if (appState.replyingToMessageId) {
          cancelReply();
        } else if (dom.manageBlocksModal?.style.display !== "none") {
          closeManageBlocksModal();
        } else if (dom.emojiPanel?.style.display !== "none") {
          hideEmojiPanel();
        } else if (
          isMobileView() &&
    ...
    dom.conversationSearchProxy?.addEventListener(
      "input",
    ...
    dom.sendButton?.addEventListener("mousedown", (e) => {
      if (isTouchDevice()) {
        e.preventDefault();
      }
    });
    dom.sendButton?.addEventListener("click", handleSendMessage);
    dom.messageInputProxy?.addEventListener(
      "keydown",
      handleMessageInputKeyDown
    );
    dom.messageInputProxy?.addEventListener("input", () => {
      updateSendButtonState();
    });
    ...
    dom.refreshMessagesButton?.addEventListener("click", () => {
      if (
        ...
        fetchMessagesForConversation(appState.currentConversationId, true);
      }
    });
    ...
    dom.manageBlocksModal
      ?.querySelector(".modal-close-button")
      ?.addEventListener("click", () => closeManageBlocksModal(null));
    dom.manageBlocksModal?.addEventListener("click", (e) => {
      if (
        e.target === dom.manageBlocksModal &&
        !e.target.closest(".glass-panel")
      )
        closeManageBlocksModal(e);
    });

    dom.adminLogoutButton?.addEventListener("click", logout);
    dom.adminAddUserForm?.addEventListener("submit", handleAdminAddUserSubmit);
    dom.adminAddUserButton?.addEventListener("click", () =>
      dom.adminAddUserForm?.requestSubmit
        ? dom.adminAddUserForm.requestSubmit()
        : dom.adminAddUserForm?.submit()
    );
    ...
    document.addEventListener("click", (e) => {
      if (
        ...
        !dom.emojiPanel?.contains(e.target) &&
        e.target !== dom.emojiToggleButton &&
        !dom.emojiToggleButton?.contains(e.target)
      ) {
        hideEmojiPanel();
      }
    });
    ...
    document.addEventListener("visibilitychange", () => {
      if (
        ...
        (appState.currentView === "chat" || appState.currentView === "admin")
      ) {
        updateActiveStatusUI();
        if (
          appState.ws &&
        ...
        if (
          ...
          fetchNewMessages(appState.currentConversationId);
        }
      } else if (
    ...
    window.addEventListener("resize", () => {
      if (!isMobileView() && dom.body.classList.contains("left-panel-active")) {
        closeMobilePanel();
      }
      adjustTextareaHeight();
      updateManualLoadButtonVisibility();
  ...
  function showUserLoginForm() {
    ...
    if (dom.loginUsernameProxy) {
      dom.loginUsernameProxy.textContent = "";
      dom.loginUsernameProxy.classList.remove("has-content");
    }
    if (dom.loginPassword) dom.loginPassword.value = "";
    if (dom.loginPasswordProxy) {
      dom.loginPasswordProxy.textContent = "";
      dom.loginPasswordProxy.classList.remove("has-content");
    }
  ...
  function showAdminLoginForm() {
    ...
    if (dom.adminMasterPasswordProxy) {
      dom.adminMasterPasswordProxy.textContent = "";
      dom.adminMasterPasswordProxy.classList.remove("has-content");
    }
