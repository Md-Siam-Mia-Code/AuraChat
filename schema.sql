...
CREATE TABLE messages (
    ...
    is_edited BOOLEAN NOT NULL DEFAULT 0 CHECK (is_edited IN (0, 1)),
...
CREATE TABLE blocks (
    blocker_id INTEGER NOT NULL, -- User initiating the block
...
CREATE INDEX IF NOT EXISTS idx_users_last_active ON users (last_active_ts DESC);
CREATE INDEX IF NOT EXISTS idx_messages_reply_to ON messages (reply_to_message_id);
CREATE INDEX IF NOT EXISTS idx_messages_sender ON messages (sender_id);

-- Initial configuration state
INSERT OR IGNORE INTO app_config (config_key, config_value) VALUES ('admin_created', 'false');
INSERT OR IGNORE INTO app_config (config_key, config_value) VALUES ('master_password_hash', ''); -- Ensure it exists
