...
export async function getAppConfig(db, key) {
  if (!db) {
    console.error("DB missing in getAppConfig");
    return null;
  }
  if (!key) {
    console.error("Config key missing in getAppConfig");
    return null;
  }
  try {
...
export async function setAppConfig(db, key, value) {
  if (!db) {
    console.error("DB missing in setAppConfig");
    return false;
  }
  if (!key || value === undefined || value === null) {
    console.error(`Invalid key/value for setAppConfig: key=${key}`);
    return false;
  }
  try {
    const stmt = db.prepare(
    ...
    if (!success) {
      console.error(`D1 statement failed for setAppConfig key '${key}'.`);
    }
    return success;
  } catch (e) {
...
export async function isBlocked(db, potentialBlockerId, potentialBlockedId) {
  if (!db || !potentialBlockerId || !potentialBlockedId) {
    console.warn("isBlocked check skipped: Missing db or IDs.");
    return false;
  }
  if (potentialBlockerId === potentialBlockedId) {
    return false;
  } // Cannot block self
  try {
    const stmt = db.prepare(
    ...
    const result = await stmt
      .bind(potentialBlockerId, potentialBlockedId)
      .first();
    return !!result;
  } catch (e) {
    console.error(
      `Error checking block status ${potentialBlockerId}->${potentialBlockedId}:`,
      e
    );
    return false;
  }
...
export async function getConversationParticipants(
  ...
  if (!db || !conversationId || isNaN(conversationId) || conversationId <= 0) {
    console.warn(
      "getConversationParticipants skipped: Missing db or invalid conversationId."
    );
    return [];
  }
  try {
    ...
    console.error(
      `Error getting participants for conversation ${conversationId}:`,
      e
    );
    return [];
  }
