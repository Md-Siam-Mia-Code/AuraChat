...
import {
  ...
  verifyJwtToken,
} from "./auth.js"; // Ensure verifyJwtToken is imported
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (!env.JWT_SECRET || env.JWT_SECRET.length < 32) {
      console.error(
        "FATAL: JWT_SECRET missing or too short. Ensure it is set in your Worker environment variables."
      );
      if (env.ENVIRONMENT !== "development") {
        // Stricter check for production
        return jsonResponse(
          {
            error:
              "Authentication configuration error. JWT_SECRET is not properly configured on the server.",
          },
          500,
          {},
          env
    ...
    try {
      ...
      if (request.method === "GET") {
        if (
          !env.ASSET_HANDLER ||
          typeof env.ASSET_HANDLER.fetch !== "function"
        ) {
          console.error("env.ASSET_HANDLER missing/invalid.");
          return jsonResponse(
            { error: "Static asset configuration error." },
            500,
            {},
            env
          );
  ...
  async handleWebSocketSession(ws, env) {
        ...
        if (data.type === "authenticate") {
          if (data.token) {
            ...
            if (authPayload) {
              ...
              console.log(
                `WS User ${currentUserId} authenticated and connection stored.`
              );
              ws.send(
            ...
            ws.send(
              JSON.stringify({ type: "auth_failed", error: "Token required" })
            );
          ...
          case "ping":
            ...
            if (env.DB && currentUserId) {
              await env.DB.prepare(
                ...
                .run()
                .catch((e) =>
                  console.error("WS Ping: Error updating last_active_ts", e)
                );
            }
            break;
          case "status_update":
            if (env.DB && data.userId && data.status && data.timestamp) {
              ...
              if (statusMap[data.status]) {
                await env.DB.prepare(
                  ...
                  .catch((e) =>
                    console.error(
                      "WS Status Update: Error updating last_active_ts",
                      e
                    )
                  );
                console.log(
                  `User ${data.userId} status updated to ${data.status} at ${statusMap[data.status]} via WS.`
                );
                ...
                broadcastUserStatus(
                  ...
                  currentUserId
                ); // currentUserId is the sender to avoid sending back
              }
            }
            break;
          case "mark_read":
            if (
              ...
              await env.DB.batch(batchStmts).catch((e) =>
                console.error("WS Mark Read: DB batch error", e)
              );
              console.log(
                `WS: Messages ${messageIds.join(", ")} in convo ${conversationId} marked as read by ${readerId}.`
              );
    ...
    ws.addEventListener("close", (event) => {
      console.log(
        `WS Connection closed for user ${currentUserId}:`,
        event.code,
        event.reason
      );
      if (currentUserId && wsConnections.has(currentUserId)) {
        ...
        if (wsConnections.get(currentUserId).size === 0) {
          ...
          console.log(
            `User ${currentUserId} fully disconnected. Updating last_active_ts.`
          );
          if (env.DB) {
            env.DB.prepare("UPDATE users SET last_active_ts = ?1 WHERE id = ?2")
              ...
              .run()
              .catch((e) =>
                console.error("WS Close: Error updating last_active_ts", e)
              );
            ...
            broadcastUserStatus(
              ...
              null
            ); // null so everyone gets it if needed
          }
        }
      }
      currentUserId = null;
    ...
    try {
      ...
      if (method === "POST" && path === "setup/admin") {
        ...
        if (
          ...
          return jsonResponse(
            { error: "Input field length requirements not met." },
            400,
            {},
            env
          );
        ...
        if (results.every((r) => r.success)) {
          ...
          console.error("Admin setup batch failed:", results);
          throw new Error("Failed to complete admin setup.");
        }
      }
      if (method === "POST" && path === "auth/login") {
        ...
        ctx.waitUntil(
          env.DB.prepare("UPDATE users SET last_active_ts = ?1 WHERE id = ?2")
            ...
            .run()
            .then(() =>
              broadcastUserStatus(user.id, true, new Date().toISOString(), null)
            )
            .catch((e) =>
              console.error(
                `Failed last_active update on login for user ${user.id}:`,
                e
              )
            )
        );
      ...
      if (method === "POST" && path === "auth/admin/login") {
        ...
        ctx.waitUntil(
          env.DB.prepare("UPDATE users SET last_active_ts = ?1 WHERE id = ?2")
            .bind(new Date().toISOString(), adminUser.id)
            .run()
            .then(() =>
              broadcastUserStatus(
                adminUser.id,
            ...
            .catch((e) =>
              console.error(
                `Failed last_active update on admin login for user ${adminUser.id}:`,
                e
              )
            )
        );
      ...
      if (method === "POST" && path === "conversations") {
        return await requireAuth(request, env, ctx, async (req) => {
          ...
          if (
            ...
            !Number.isInteger(body.partnerId) ||
            body.partnerId <= 0
          ) {
            return jsonResponse(
              { error: "Partner ID must be a positive integer." },
              400,
              {},
              env
            );
          ...
          if (partner.is_admin)
            return jsonResponse(
              { error: "Cannot start conversation with an admin." },
              403,
              {},
              env
            );
          ...
          if (
            (await isBlocked(env.DB, partnerId, userId)) ||
            (await isBlocked(env.DB, userId, partnerId))
          ) {
            return jsonResponse(
              { error: "Cannot start chat due to blocking." },
              403,
              {},
              env
            );
          ...
          try {
            ...
            if (!result || typeof result.id !== "number") {
              throw new Error(
                "Failed to create conversation row or retrieve ID."
              );
            }
            ...
            if (!batchResult.every((r) => r.success)) {
              console.error(
                `Failed to add participants for new convo ${newConversationId}:`,
                batchResult
              );
              await env.DB.prepare("DELETE FROM conversations WHERE id = ?1")
                .bind(newConversationId)
                .run()
                .catch((cleanErr) =>
                  console.error(
                    `Cleanup failed convo ${newConversationId}:`,
                    cleanErr
                  )
                );
            ...
            console.error(
              `Error creating conversation between ${userId} and ${partnerId}:`,
              e
            );
            if (
              newConversationId &&
              e.message?.includes("Failed to add participants")
            ) {
              await env.DB.prepare("DELETE FROM conversations WHERE id = ?1")
                .bind(newConversationId)
                .run()
      ...
      if (
        ...
        return await requireAuth(request, env, ctx, async (req) => {
          ...
          const userId = req.auth.userId;
          const participantCheck = await env.DB.prepare(
          ...
          if (sinceTimestamp) {
            if (isNaN(new Date(sinceTimestamp).getTime()))
              return jsonResponse(
            ...
            if (isNaN(new Date(beforeTimestamp).getTime()))
              return jsonResponse(
                { error: "Invalid 'before_ts' timestamp format." },
                400,
                {},
                env
              );
            query = `${baseSelect} WHERE m.conversation_id = ?3 AND m.timestamp < ?4 ORDER BY m.timestamp DESC LIMIT ?5;`;
          ...
          let messagesData = (results || []).map((msg) => ({
            ...msg,
            isReadByPartner: !!msg.isReadByPartner,
            is_edited: !!msg.is_edited,
          }));
          if (!sinceTimestamp) messagesData.reverse();
          const receivedMessageIds = messagesData
            .filter((m) => m.sender_id !== userId)
            .map((m) => m.id);
          if (receivedMessageIds.length > 0 && !beforeTimestamp) {
            ctx.waitUntil(
              (async () => {
                try {
                  const batchStmts = receivedMessageIds.map((msgId) =>
                    env.DB.prepare(
                      "INSERT OR IGNORE INTO message_read_status (message_id, user_id) VALUES (?1, ?2)"
                    ).bind(msgId, userId)
                  );
                  ...
                  console.error(
                    `Error marking messages read for user ${userId}, convo ${conversationId}:`,
                    dbErr
                  );
                }
              })()
      ...
      if (
        ...
        return await requireAuth(request, env, ctx, async (req) => {
          ...
          const validationError = validateInput(body, ["content"]);
          if (validationError)
            return jsonResponse({ error: validationError }, 400, {}, env);
          ...
          if (!content || content.length > 1000)
            return jsonResponse(
              { error: "Content required (1-1000 chars)." },
              400,
              {},
              env
            );
          if (
            ...
            (isNaN(replyToMessageId) || replyToMessageId <= 0)
          ) {
            return jsonResponse(
          ...
          if (replyToMessageId) {
            const repliedMsgExists = await env.DB.prepare(
              "SELECT 1 FROM messages WHERE id = ?1 AND conversation_id = ?2"
            )
              .bind(replyToMessageId, conversationId)
              .first();
            if (!repliedMsgExists)
              return jsonResponse(
                {
                  error:
                    "Message being replied to not found in this conversation.",
                },
                404,
                {},
                env
          ...
          try {
            const result = await env.DB.prepare(
              ...
              .first();
            if (!result || typeof result.id !== "number") {
              throw new Error("Failed to insert message or retrieve ID.");
            }
            ...
            ctx.waitUntil(
              env.DB.prepare(
                ...
                .catch((err) =>
                  console.error(
                    `Failed update convo ts ${conversationId}:`,
                    err
                  )
                )
            );
            ...
            const newMessageData = await fetchStmt
              .bind(newMessageId, conversationId, userId, username)
              .first();
            if (!newMessageData) {
              throw new Error("Failed to retrieve newly inserted message.");
            }
            ...
            console.error(
              `Error sending msg user ${userId} convo ${conversationId}:`,
              e
            );
            if (newMessageId) {
              await env.DB.prepare("DELETE FROM messages WHERE id = ?1")
                .bind(newMessageId)
                .run()
                .catch((delErr) =>
                  console.error(
                    `Failed cleanup for partially failed message send ${newMessageId}:`,
                    delErr
                  )
                );
            }
      ...
      if (
        ...
        return await requireAuth(request, env, ctx, async (req) => {
          ...
          const messageIdsToMark = body?.message_ids;
          if (
            !Array.isArray(messageIdsToMark) ||
            messageIdsToMark.some((id) => isNaN(parseInt(id, 10)))
          ) {
            return jsonResponse(
              { error: "Invalid message_ids provided." },
              400,
              {},
              env
            );
          ...
          if (messageIdsToMark.length === 0) {
            return jsonResponse(
              { success: true, message: "No messages to mark." },
              200,
              {},
              env
            );
          }
          const placeholders = messageIdsToMark.map(() => "?").join(",");
          const stmt = env.DB.prepare(
            ` INSERT OR IGNORE INTO message_read_status (message_id, user_id) SELECT m.id, ?1 FROM messages m WHERE m.conversation_id = ?2 AND m.sender_id != ?1 AND m.id IN (${placeholders}) `
          );
          ctx.waitUntil(
            (async () => {
              try {
                ...
                const { success, meta } = await stmt.bind(...bindParams).run();
                if (success && meta.changes > 0) {
                  ...
                  console.log(
                    `Marked ${meta.changes} msgs as read by user ${userId} via /read endpoint for convo ${conversationId}`
                  );
                } else if (!success) {
                  console.error(
                    `DB error marking messages read via /read for user ${userId}, convo ${conversationId}`
                  );
                }
              } catch (dbErr) {
                console.error(
                  `Error executing mark messages read via /read endpoint:`,
                  dbErr
                );
              }
            })()
      ...
      if (
        ...
        return await requireAuth(request, env, ctx, async (req) => {
          ...
          if (isNaN(messageId) || messageId <= 0)
            return jsonResponse({ error: "Invalid message ID." }, 400, {}, env);
          const userId = req.auth.userId;
          const validationError = validateInput(body, ["content"]);
          if (validationError)
            return jsonResponse({ error: validationError }, 400, {}, env);
          ...
          const originalMessage = await env.DB.prepare(
            "SELECT sender_id, conversation_id, timestamp, content FROM messages WHERE id = ?1"
          )
            .bind(messageId)
            .first();
          if (!originalMessage)
          ...
          const { success, meta } = await env.DB.prepare(
            "UPDATE messages SET content = ?1, is_edited = 1, edited_at = ?2 WHERE id = ?3 AND sender_id = ?4"
          )
            .bind(newContent, now, messageId, userId)
            .run();
          if (success && meta.changes > 0) {
            console.log(`User ${userId} edited message ${messageId}`);
            ctx.waitUntil(
              env.DB.prepare(
                `UPDATE conversations SET last_activity_ts = ?1 WHERE id = ?2 AND (SELECT MAX(m.timestamp) FROM messages m WHERE m.conversation_id = ?2) = ?3`
              )
                .bind(
                  now,
                ...
                .catch((err) =>
                  console.error(
                    `Failed update convo ts on edit ${originalMessage.conversation_id}:`,
                    err
                  )
                )
            );
            return jsonResponse(
              { success: true, edited_at: now },
              200,
              {},
              env
            );
            ...
            console.error(
              `Failed to update message ${messageId} for user ${userId}`
            );
      ...
      if (method === "POST" && path === "blocks") {
        return await requireAuth(request, env, ctx, async (req) => {
          ...
          if (
            ...
            return jsonResponse(
              { error: "Invalid user ID to block." },
              400,
              {},
              env
            );
          ...
          const userToBlock = await env.DB.prepare(
            "SELECT id, is_admin, username FROM users WHERE id = ?1"
          )
            .bind(blockedId)
            .first();
          if (!userToBlock)
            return jsonResponse({ error: "User not found." }, 404, {}, env);
          if (userToBlock.is_admin)
            return jsonResponse(
              { error: "Cannot block an administrator." },
              403,
              {},
              env
            );
          ...
          if (success) {
            if (meta.changes > 0)
              console.log(
                `User ${userId} blocked user ${userToBlock.username} (ID: ${blockedId})`
              );
            return jsonResponse({ success: true }, 201, {}, env);
          } else {
            console.error(
              `Failed to block user ${blockedId} for user ${userId}`
            );
      ...
      if (
        method === "DELETE" &&
        pathSegments.length === 2 &&
        pathSegments[0] === "blocks"
      ) {
        return await requireAuth(request, env, ctx, async (req) => {
          const blockerId = req.auth.userId;
          const blockedIdToUnblock = parseInt(pathSegments[1], 10);
          if (isNaN(blockedIdToUnblock) || blockedIdToUnblock <= 0) {
            return jsonResponse(
              { error: "Invalid user ID to unblock." },
              400,
              {},
              env
            );
          ...
          if (success) {
            const message =
              meta.changes > 0
                ? "User unblocked."
                : "User was not blocked by you.";
            if (meta.changes > 0)
              console.log(
                `User ${blockerId} unblocked user ${blockedIdToUnblock}`
              );
            return jsonResponse(
            ...
            console.error(
              `Failed to unblock user ${blockedIdToUnblock} for user ${blockerId}`
            );
        ...
        if (method === "GET" && path === "admin/stats") {
          return await requireAdminAuth(request, env, ctx, async (req) => {
            console.log(
              `Admin stats request by admin ${req.auth.userId} (${req.auth.username})`
            );
            const [
              userCountRes,
              messageCountRes,
        ...
        if (method === "GET" && path === "admin/users") {
          return await requireAdminAuth(request, env, ctx, async (req) => {
            console.log(
              `Admin user list request by admin ${req.auth.userId} (${req.auth.username})`
            );
            const { results } = await env.DB.prepare(
          ...
          return await requireAdminAuth(request, env, ctx, async (req) => {
            ...
            try {
              ...
              if (result && typeof result.id === "number") {
                ...
                throw new Error("DB Insert failed or did not return ID.");
              }
            } catch (e) {
              if (
              ...
              console.error(
                `Admin ${adminUserId} (${req.auth.username}) failed to create user:`,
                e
              );
              throw e;
            }
        ...
        if (
          ...
          return await requireAdminAuth(request, env, ctx, async (req) => {
            ...
            if (isNaN(userIdToDelete) || userIdToDelete <= 0) {
              return jsonResponse({ error: "Invalid user ID." }, 400, {}, env);
            }
            ...
            if (success && meta.changes > 0) {
              console.log(
                `Admin ${adminUserId} (${req.auth.username}) deleted user ${userToDelete.username} (ID: ${userIdToDelete})`
              );
              return new Response(null, { status: 204 });
            } else if (success) {
              return jsonResponse(
              ...
              console.error(
                `Admin ${adminUserId} (${req.auth.username}) failed to delete user ${userIdToDelete}`
              );
    ...
    userConnections.forEach((ws) => {
      if (ws.readyState === WebSocket.OPEN) {
        try {
          ...
          console.error("Error sending WS message to user", userId, e);
        }
      }
    });
  }
