...
export function jsonResponse(body, status = 200, headers = {}, env = null) {
  const allowedOrigin =
    env && env.FRONTEND_ORIGIN && env.ENVIRONMENT !== "development"
      ? env.FRONTEND_ORIGIN
...
export function validateInput(data, requiredFields) {
  ...
  if (
    data.masterPassword !== undefined &&
    (typeof data.masterPassword !== "string" || data.masterPassword.length < 10)
  ) {
  ...
  if (
    ...
    (typeof data.partnerId !== "number" ||
      !Number.isInteger(data.partnerId) ||
      data.partnerId <= 0)
  ) {
    return "Partner ID must be a positive integer.";
  }
  if (
    data.reply_to_message_id !== undefined &&
    data.reply_to_message_id !== null &&
    (typeof data.reply_to_message_id !== "number" ||
      !Number.isInteger(data.reply_to_message_id) ||
